// File [Model_java_BREAKOUTSwing.html] MAS - last created: Fri 28 Apr 2017 21:40:20 BST
...
public class Model extends Observable
{
    ...
    private static final float The_Depth = 3; 
    private static final float NumOfBricks = 8;
    ...
    public void createGameObjects()
    {
        synchronized( Model.class )
        {
            ...
            float PixelT = 15; 
            float pixelThree = 15;
            for(int j = 0; j< The_Depth; j++){
                for(int i = 0; i < NumOfBricks; i++){
                    bricks.add(new GameObj((W/NumOfBricks)* i + 10,(H/2/The_Depth)*j + (H/10),BRICK_WIDTH,BRICK_HEIGHT,Colour.GRAY)); 
                } 
            }
            //13
    ...
    public void moveBat( int direction )
    {
        ...
        if ((dist < 0 && bat.getX() < 5) || (dist > 0 && bat.getX() > 445))
            return;
        bat.moveX(dist);
